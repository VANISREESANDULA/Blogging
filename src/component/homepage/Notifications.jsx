...
import {Heart,MessageCircle,UserPlus,AtSign,Bell,Search,Check,X,Clock,User,
  ExternalLink,Filter,MoreVertical,UserCheck, UserX,Send,Users,ThumbsUp,MessageSquare,Hash,AlertCircle,} from "lucide-react";
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="flex-1">
              <p className={`font-medium ${isDark ? "text-gray-200" : "text-gray-900"} mb-1 break-words whitespace-normal`}> 
                {n.message}
              </p>
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-8`}>
              {tabs.map(tab => (
                <div 
                  ...
                  className={`p-4 rounded-xl border cursor-pointer transition-all duration-300 hover:scale-105 min-w-0 ${
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className={`w-10 h-10 rounded-full bg-gradient-to-r ${tab.color} flex items-center justify-center flex-shrink-0`}>
                      {tab.icon}
                    </div>
                    <span className={`text-2xl font-bold truncate ${isDark ? "text-gray-200" : "text-gray-900"}`}>
                      {stats[tab.key] || 0}
                    </span>
                  </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 min-w-0">
              <div className={`rounded-xl border min-w-0 overflow-hidden ${isDark ? "border-gray-700 bg-gray-800" : "border-orange-200 bg-white"}`}>
                {/* Header */}
                <div className={`p-5 border-b ${isDark ? "border-gray-700" : "border-orange-200"}`}>
                  ...
                  {/* Tabs */}
                  <div className="flex flex-wrap md:flex-nowrap gap-2 overflow-x-auto pb-2">
                    {tabs.map((tab) => (
                        <button
                        key={tab.key}
                        onClick={() => setActiveFilter(tab.key)}
                        className={`flex items-center gap-2 px-4 py-2.5 mx-1 rounded-lg text-sm font-medium transition-all duration-300 ${
                          activeFilter === tab.key
                            ? isDark
