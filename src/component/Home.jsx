...
const Home = () => {
  ...
  const { followers, following } = useSelector((state) => state.global);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      ...
      {/* ==================== DESKTOP SIDEBAR ==================== */}
      <div className="hidden md:flex h-screen w-64 bg-white shadow-md flex-col">
        <div className="p-4 border-b">
          <h1 className="text-2xl font-semibold text-purple-700">EveryTale</h1>
        </div>

        <div className="p-4 flex flex-col gap-4 text-gray-700">
          <button onClick={() => navigate("/home")} className="flex items-center gap-2">🏠 Home</button>
          <button onClick={() => navigate("/add-friends")} className="flex items-center gap-2">👥 Add Friends</button>
          <button className="flex items-center gap-2">📰 News</button>
          <button className="flex items-center gap-2">⚽ Sports</button>
          <button className="flex items-center gap-2">🎵 Music</button>
          <button  onClick={() => navigate("/Bookmarks")}className="flex items-center gap-2">Bookmarks</button>
          <button onClick={() => navigate("/SettingsPage")} className="flex items-center gap-2">⚙️ Settings</button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col md:ml-0 ml-0">
        ...
        >
          <div className="flex items-center gap-3 ml-12 md:ml-0">
            <img src="/Profile Icon.png" alt="Profile" className="w-20 h-20 rounded-full" />
            <div className="text-white">
              <h2 className="font-bold">Raju Kumar</h2>
              <p className="text-sm opacity-90">{followers} Followers · {following} Following</p>
            </div>
          </div>
          ...
          <div className="w-1/3 hidden md:block">
            <input
              type="text"
              placeholder="🔍Search..."
              className="w-full px-3 py-2 border-2 rounded-full text-black shadow bg-orange-100/50"
            />
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Posts */}
          {posts.map((post) => (
            <div key={post.id} className="bg-white p-4 rounded-lg shadow-md flex flex-col gap-2">
              <div className="flex items-center gap-3">
                <img src="/Profile Icon.png" alt="User" className="w-10 h-10 rounded-full" />
                <div>
                  <h4 className="font-semibold">{post.name}</h4>
                  <p className="text-sm text-gray-500">{post.time}</p>
                </div>
      ...
      {/* =================== INSTAGRAM STYLE BOTTOM NAV (MOBILE) =================== */}
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-black/40 backdrop-blur-xl flex justify-around py-3 text-3xl text-white">

        <button onClick={() => navigate("/home")}>🏠</button>
        <button onClick={() => navigate("/add-friends")}>👥</button>
        <button>🔍</button>
        <button>🎵</button>
        <button onClick={() => navigate("/SettingsPage")}>⚙️</button>
